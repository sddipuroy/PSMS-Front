...
$cureent_class_status = Student('current_class',$_SESSION['st_loggedin'][0]['id']);
if($cureent_class_status != null){
	$current_date = date('Y-m-d');
	$stm=$pdo->prepare("SELECT * FROM class WHERE start_date <=? AND end_date >=? AND id=?");
	$stm->execute(array($current_date,$current_date,$cureent_class_status));
				...
				<div class="col-lg-12 m-b30">
					<div class="widget-box">
						...
						<?php 
							...
							$stm->execute(array($cureent_class_status));
						...
						<?php else: ?>
							<div class="alert alert-danger">
								Please Frist Register Your Class then you will get the class routine.
							</div>
						<?php endif; ?>
					</div>
				</div>
